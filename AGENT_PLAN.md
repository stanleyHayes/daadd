...
- Phase 5 escrow settlement — `SETTLEMENT_MODE` config: `escrow` (default; hold on platform, transfer the merchant's share on order completion via a Paystack transfer recipient — true per-order buyer-protection hold) or `split` (subaccount split at charge). `Order.settlement_status` tracks held→released/refunded. Provider gains transfer-recipient + transfer. Backend + 3 escrow tests.
- Phase 5 settlement (self-service subaccounts) — merchants connect their own bank/mobile-money on the dashboard (resolve/verify → create Paystack subaccount); order charges split so the merchant's share settles to them directly (DAADD holds neither customer nor merchant funds). Backend + merchant UI + 4 tests, six locales.
