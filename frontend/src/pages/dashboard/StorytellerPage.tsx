...
export function StorytellerPage() {
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <PageHeader
        title={t('dashboard.storyteller.title')}
        subtitle={t('dashboard.storyteller.subtitle')}
        action={
          campaignId && (
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" icon={<FileText className="h-4 w-4" />} onClick={() => { exportPDF.mutate(campaignId); toast.success(t('dashboard.storyteller.exportingPdf')); }} loading={exportPDF.isPending}>
                {t('dashboard.common.exportPdf')}
              </Button>
              <Button variant="outline" size="sm" icon={<Code className="h-4 w-4" />} onClick={() => { exportHTML.mutate(campaignId); toast.success(t('dashboard.storyteller.exportingHtml')); }} loading={exportHTML.isPending}>
                {t('dashboard.common.exportHtml')}
              </Button>
            </div>
          )
        }
      ...
      {campaignId && story && !isLoading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricsCard icon={<BookOpen className="h-5 w-5" />} label={t('dashboard.storyteller.chapters')} value={String(chapters.length)} iconColor="text-primary-600" iconBg="bg-primary-50 dark:bg-primary-900/30" />
          <MetricsCard icon={<Lightbulb className="h-5 w-5" />} label={t('dashboard.storyteller.keyInsights')} value={String(insights.length)} iconColor="text-secondary-600" iconBg="bg-secondary-50 dark:bg-secondary-900/30" />
          <MetricsCard icon={<ArrowRight className="h-5 w-5" />} label={t('dashboard.storyteller.recommendations')} value={String(recommendations.length)} iconColor="text-accent-600" iconBg="bg-accent-50 dark:bg-accent-900/30" />
          <MetricsCard icon={<Clock className="h-5 w-5" />} label={t('dashboard.storyteller.campaignAge')} value={campaignAge ? `${Math.round(campaignAge)}h` : 'N/A'} iconColor="text-warning-600" iconBg="bg-warning-50 dark:bg-warning-900/30" />
        </div>
      )}
      ...
      {!campaignId ? (
        <Card>
          <div className="text-center py-16">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">{t('dashboard.storyteller.selectPrompt')}</p>
          </div>
        </Card>
      ) : isLoading ? (
        ...
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">{t('dashboard.storyteller.loadError')}</p>
          </div>
        </Card>
        ...
        <>
          ...
          {insights.length > 0 && (
            <Card>
              <CardHeader title={t('dashboard.storyteller.keyInsights')} />
          ...
          {recommendations.length > 0 && (
            <Card>
              <CardHeader title={t('dashboard.storyteller.recommendations')} />
              <div className="space-y-3">
